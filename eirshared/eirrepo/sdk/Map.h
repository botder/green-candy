...
struct Node
{
    MAP_TEMPLARGS friend struct ::eir::Map;
    ...
    struct avlKeyNodeDispatcher
    {
        ...
        // Fuck off, MSVC! You seriously annoy me. Microsoft, improve your lexer!! Your product is stepping backwards in quality!
        static inline eCompResult CompareNodes( const AVLNode *left, const AVLNode *right ) _MSVC_BUGFIX(noexcept(std::is_nothrow_invocable <decltype(compare_keys <keyType, keyType>), keyType, keyType>::value))
        {
        ...
        static inline eCompResult CompareNodeWithValue( const AVLNode *left, const subKeyType& right ) _MSVC_BUGFIX(noexcept(std::is_nothrow_invocable <decltype(compare_keys <keyType, subKeyType>), keyType, subKeyType>::value))
        {
            const Node *leftNode = AVL_GETITEM( Node, left, sortedByKeyNode );

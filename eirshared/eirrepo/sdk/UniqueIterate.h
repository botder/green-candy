...
#ifdef _MSC_VER
        goto next; // shut up about next; I had to fix this because MSVC constexpr-branch label detection is broken thus jumping inside
        // constexpr branches is always invalid! Microsoft's compiler is seriously degrading in quality over time.
    next:
#endif
        container_idx++;
        return false;

...
struct ValueNode
{
    SET_TEMPLARGS friend struct ::eir::Set;
    ...
    struct avlKeyNodeDispatcher
    {
        ...
        // Fuck off, MSVC. You seriously annoy me!
        static inline eCompResult CompareNodes( const AVLNode *left, const AVLNode *right ) _MSVC_BUGFIX(noexcept(std::is_nothrow_invocable <decltype(compare_values <valueType, valueType>), valueType, valueType>::value))
        {
        ...
        static inline eCompResult CompareNodeWithValue( const AVLNode *left, const subValueType& right ) _MSVC_BUGFIX(noexcept(std::is_nothrow_invocable <decltype(compare_values <valueType, subValueType>), valueType, subValueType>::value))
        {
            const ValueNode *leftNode = AVL_GETITEM( ValueNode, left, sortedByValueNode );

...
void DirectX9Texture::loadFromFile(const String& filename, const String& resourceGroup)
{
	...
	{
        // Create an ANSI Direct3D9 error string.
        const wchar_t *dxerrstr = DXGetErrorStringW( hr );

        std::string ansiErrorString = CharacterUtil::ConvertStrings <wchar_t, char> ( dxerrstr );

		throw RendererException((utf8*)"Failed to create Texture object from file '" + filename + "'.  Additional Info: " + ansiErrorString);
	}

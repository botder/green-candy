...
SString& SString::vFormat ( const char* szFormat, va_list vl )
{
    ...
    va_copy ( vlLocal, vl );
    int iSize = vsnprintf ( szDest, iRequiredCapacity + 1, szFormat, vlLocal );
...
// Split into parts
//
void SString::Split ( const SString& strDelim, std::vector < SString >& outResult, size_t uiMaxAmount, size_t uiMinAmount ) const
{
    outResult.clear ();
...
{
    SString strResult;
    int iLast = Min < int > ( iFirst + iCount, (int)parts.size () ) - 1;

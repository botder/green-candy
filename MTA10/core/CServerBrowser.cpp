...
bool CServerBrowser::OnAddressChanged ( CGUIElement* pElement )
{
    ...
    for ( unsigned int i = 0; i < SERVER_BROWSER_TYPE_COUNT; i++ )
    {
        if ( ( i != Type ) && ( strURI != std::string( m_pEditAddress[i]->GetText() ) ) )
...
bool CServerBrowser::OnSearchTypeSelected ( CGUIElement* pElement )
{
    ...
    if ( std::string( m_pEditSearch [ Type ]->GetText() ).empty() )
        return true;
...
bool CServerBrowser::OnMouseClick ( CGUIMouseEventArgs Args )
{
    ...
    else if ( Args.pWindow == m_pServerPlayerList [ Type ] && m_pComboSearchType[ Type ]->GetSelectedItemIndex() == SearchTypes::PLAYERS && !std::string( m_pEditSearch [ Type ]->GetText () ).empty() )
    {
...
void CServerBrowser::SetNextHistoryText ( bool bDown )
{
    ...
    for ( int i = 0 ; i < iRowCount ; i++ )
    {
        CGUIListItem* item = pServerList->GetItemByIndex(i);
        const char* szAddress = (const char*)item->GetData();    
        if ( ("mtasa://" + std::string(szAddress)) == std::string( m_pEditAddress[ Type ]->GetText() ) )
        {
            // We've found our row
            i = bDown ? i+1 : i-1;

...
bool IsPng ( const void* pData, uint uiDataSize )
{
    ...
    {
        if ( memcmp ( pData, PngHeader, sizeof ( PngHeader ) ) == 0 &&
             memcmp ( (BYTE*)pData + uiDataSize - sizeof ( PngTail ), PngTail, sizeof ( PngTail ) ) == 0 )
        {
            return true;
        }
    }
...
{
    CBufferReadStream& stream = *(CBufferReadStream*)png_get_io_ptr( png_ptr );
...
{
    CBufferWriteStream* pStream=(CBufferWriteStream*)png_get_io_ptr( png_ptr );

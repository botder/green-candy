...
void CVersionUpdater::InitiateManualCheck ( void )
{
    ...
    strUpdateBuildType = CVARS_GET ( "update_build_type" );
...
void CVersionUpdater::_ResetManualCheckTimer ( void )
{
    ...
    m_strLastManualCheckBuildType = CVARS_GET ( "update_build_type" );
}
...
void CVersionUpdater::_DialogUpdateResult(void)
{
    ...
    if ( m_ConditionMap.IsConditionTrue ( "Download.Ok" ) )
    {
        ...
        if ( m_JobInfo.strStatus == "update" )
        {
            ...
            SetOnQuitCommand ( "open", m_JobInfo.strSaveLocation.convert_ansi() );
            SetOnRestartCommand ( "" );
            Push ( _ExitGame );
        }
        else
        if ( m_JobInfo.strStatus == "files" || m_JobInfo.strStatus == "silent" )
        {
            ...
            SetOnRestartCommand ( "files", m_JobInfo.strSaveLocation.convert_ansi() );
            Push ( _ExitGame );
        }
        else
        {
...
void CVersionUpdater::_QUpdateResult ( void )
{
    if ( m_ConditionMap.IsConditionTrue ( "Download.Ok" ) )
    {
        ...
        if ( m_JobInfo.strStatus == "silent" )
        {
            ...
            SetOnRestartCommand ( "silent", m_JobInfo.strSaveLocation.convert_ansi() );
        }
#if MTA_DEBUG
        else
    ...
    if ( m_ConditionMap.IsConditionTrue ( "Download.Ok" ) )
    {
        ...
        if ( m_JobInfo.strStatus == "silent" )
        {
            ...
            if ( strDate.length () == 10 )
            {
                ...
                queue.Set ( strDate, m_JobInfo.strSaveLocation.convert_ansi() );
...
//
///////////////////////////////////////////////////////////////
struct _scandir_finish_exception
{};
...
void CVersionUpdater::_StartDownload ( void )
{
    if ( !m_JobInfo.strFilename.empty() )
    {
        ...
        for ( std::list <filePath>::iterator iter = saveLocationList.begin(); iter != saveLocationList.end(); ++iter )
        {
            ...
            std::string ansi_path = ( *iter ).convert_ansi();

            ExtractFilename ( ansi_path, &strPath, &strFilename );
            ExtractExtention ( ansi_path, &strMain, NULL );

            CFileTranslator *minimumAccessRoot = fileSystem->CreateSystemMinimumAccessPoint( ansi_path.c_str() );

            if ( minimumAccessRoot )
            {
                try
                {
                    minimumAccessRoot->ScanDirectory( ansi_path.c_str(), "*", true,
                        NULL,
                        [&]( const filePath& absPath )
                    {
                        ...
                        // Check filesize
                        std::string ansiPathFilename = strPathFilename.convert_ansi();

                        if ( g_pCore->GetFileSystem()->Size( ansiPathFilename.c_str() ) != m_JobInfo.iFilesize )
                            return;

                        g_pCore->GetFileSystem()->ReadToBuffer( ansiPathFilename.c_str(), buffer );
                        ...
                        CMD5Hasher().Calculate( buffer.data(), buffer.size(), md5Result );
                        CMD5Hasher::ConvertToHex( md5Result, szMD5 );

                        if ( m_JobInfo.strMD5 != szMD5 )
                        {
                            AddReportLog ( 5807, SString ( "StartDownload: Cached file reuse - Size correct, but md5 did not match (%s)", ansiPathFilename.c_str() ) );
                            return;
                        }
                        ...
                        {
                            AddReportLog ( 5808, SString ( "StartDownload: Cached file reuse - Size and md5 correct, but signature incorrect (%s)", ansiPathFilename.c_str() ) );
                            return;
                        }
                        ...
                        m_ConditionMap.SetCondition ( "ProcessResponse", "" );
                        m_ConditionMap.SetCondition ( "Download", "Ok" );
                        AddReportLog ( 5809, SString ( "StartDownload: Cached file reuse - Size, md5 and signature correct (%s)", ansiPathFilename.c_str() ) );
                    
                        throw _scandir_finish_exception(); // WE HAVE TO BREAK SCANDIR HERE.
                    }, NULL);
                }
                catch( _scandir_finish_exception& )
                {
...
int CVersionUpdater::DoSendDownloadRequestToNextServer ( void )
{
    ...
    SString strUpdateBuildType = CVARS_GET ( "update_build_type" );

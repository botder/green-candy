...
public:
    ...
    const char*     Get                     ( const std::string& strVariable )                               { SANGET; return Node(strVariable)->GetTagContent (); }
    bool            Get                     ( const std::string& strVariable, std::string& strOut )          { SANGET; strOut = Node(strVariable)->GetTagContent (); return strOut.empty(); }

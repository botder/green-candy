...
{
    if ( std::string( m_pEditUsername->GetText() ).empty() )
        g_pCore->ShowMessageBox ( "Error", "Username missing", MB_BUTTON_OK | MB_ICON_INFO );
    else if ( std::string( m_pEditEmail->GetText() ).empty() )
        g_pCore->ShowMessageBox ( "Error", "Email missing", MB_BUTTON_OK | MB_ICON_INFO );
    else if ( std::string( m_pEditPassword->GetText() ).empty() || std::string( m_pEditConfirm->GetText() ).empty() )
        g_pCore->ShowMessageBox ( "Error", "Password missing", MB_BUTTON_OK | MB_ICON_INFO );
    else if ( std::string( m_pEditPassword->GetText() ) != std::string( m_pEditConfirm->GetText() ) )
        g_pCore->ShowMessageBox ( "Error", "Passwords do not match", MB_BUTTON_OK | MB_ICON_INFO );
    else if ( std::string( m_pEditCode->GetText() ).empty() )
    ...
    else
    {
        if ( m_ulStartTime == 0 )
        {
            ...
            std::string strURL =
                ...
                "?action=register" +
                "&username=" + std::string( m_pEditUsername->GetText() ) + 
                "&password=" + strPassword +
                "&email=" + std::string( m_pEditEmail->GetText() ) +
                "&code=" + std::string( m_pEditCode->GetText() ) +

...
bool CCommunityLogin::OnButtonLoginClick ( CGUIElement* pElement )
{
    if ( std::string( m_pEditUsername->GetText() ).empty() ||
        std::string( m_pEditPassword->GetText() ).empty() )
    {
    ...
    MD5 Password;
    CMD5Hasher Hasher;
    std::string editPassword = m_pEditPassword->GetText();
    Hasher.Calculate ( editPassword.c_str(), editPassword.length(), Password );

...
bool CSettings::OnLoginButtonClick ( CGUIElement* pElement )
{
    ...
    {
        if ( std::string( m_pEditUser->GetText() ).empty() ||
            std::string( m_pEditPass->GetText() ).empty() )
        {
        ...
        else
        {
            ...
            MD5 Password;
            CMD5Hasher Hasher;
            std::string editPass( m_pEditPass->GetText() );
            Hasher.Calculate ( editPass.c_str(), editPass.length(), Password );
            ...
            if ( std::string( m_pEditPass->GetText () ).compare ( strCommunityPassword ) == 0 )
...
void CSettings::LoadData ( void )
{
    ...
    const char *text = m_pInterfaceSkinSelector->GetItemText( uiIndex );
    if ( text )
    {
        std::string strItemText = text;
...
void CSettings::SaveData ( void )
{
    ...
    {
        CVARS_GET ( "nick", strVar );
        if ( std::string( m_pEditNick->GetText () ).compare ( strVar ) != 0 )
    ...
    strVar = std::string( m_pChatScaleX->GetText () ) + " " + std::string( m_pChatScaleY->GetText () );
    CVARS_SET ( "chat_scale", strVar );

    // Test error reporter
    if ( std::string( m_pChatLines->GetText () ) == "crashme" )
        *((int*)NULL) = 0;      

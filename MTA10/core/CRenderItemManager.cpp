...
//
////////////////////////////////////////////////////////////////
CShaderItem* CRenderItemManager::CreateShader ( const SString& strFullFilePath, const SString& strRootPath, abiString& strOutStatus, float fPriority, float fMaxDistance, bool bDebug )
{
    if ( !CanCreateRenderItem ( CShaderItem::GetClassId () ) )
        return NULL;

    SString outStrStatus = "";

    CShaderItem* pShaderItem = new CShaderItem ();
    pShaderItem->PostConstruct ( this, strFullFilePath, strRootPath, outStrStatus, fPriority, fMaxDistance, bDebug );

    strOutStatus = outStrStatus;

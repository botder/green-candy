...
ID3DXEffect* CEffectClonerImpl::CreateD3DEffect ( const SString& strFilename, const SString& strRootPath, SString& strOutStatus, bool bDebug )
{
    ...
    SString lowerName = strFilename.ToLower();
    CEffectTemplate* pEffectTemplate = MapFindRef ( m_ValidMap, lowerName );
    if ( pEffectTemplate )
    {
        ...
        if ( pEffectTemplate->HaveFilesChanged () )
        {
            ...
            MapRemove ( m_ValidMap, lowerName );
    ...
    if ( !pEffectTemplate )
    {
        ...
        // Add to active map
        MapSet ( m_ValidMap, lowerName, pEffectTemplate );
    }

#ifndef _CBUFFER_LOGIC_
#define _CBUFFER_LOGIC_
...
public:
    ...
    unsigned int            Version ( void ) const          { return m_uiVersion; }
...
public:
    ...
    void WriteString ( const SString& str, bool bByteLength = false, bool bDoesLengthIncludeLengthOfLength = false )
    {
        ...
        assert( strlen < std::numeric_limits <ushort>::max() );

        ushort usLength = (ushort)strlen;
        if ( bDoesLengthIncludeLengthOfLength && usLength )
            usLength += bByteLength ? 1 : 2;

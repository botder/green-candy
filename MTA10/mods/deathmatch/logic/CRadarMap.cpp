...
CRadarMap::CRadarMap ( CClientManager* pManager )
{
    ...
    m_pRadarImage = g_pCore->GetGraphics()->GetRenderItemManager()->CreateTexture( ( imgPath + "radar.jpg" ).c_str(), NULL, false, 1024, 1024, RFORMAT_DXT1 );
    m_pLocalPlayerBlip = g_pCore->GetGraphics()->GetRenderItemManager()->CreateTexture( ( imgPath + "radarset/02.png" ).c_str() );
    ...
    {
        CClientTextDisplay* pTextDisplay = new CClientTextDisplay ( m_pManager->GetDisplayManager (), 0xffffffff );
...
void CRadarMap::CreateMarkerTextures()
{
    ...
    mtaFileRoot->ChangeDirectory( "cgui/images/radarset/" );
    ...
    {
        filePath path;
        mtaFileRoot->GetFullPath( shapeFileNames[i], true, path );

        CTextureItem* pTextureItem = g_pCore->GetGraphics()->GetRenderItemManager ()->CreateTexture ( path.c_str() );
    ...
    {
        filePath path;
        mtaFileRoot->GetFullPath( SString( "%02u.png", i + 1 ).c_str(), true, path );

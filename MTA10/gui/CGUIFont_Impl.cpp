...
CGUIFont_Impl::CGUIFont_Impl( CGUI_Impl* pGUI, const char* szFontName, const filePath& szFontFile, unsigned int uSize, unsigned int uFlags, bool bAutoScale )
{
    ...
    while ( !m_pFont )
    {
        try
        {
            m_pFont = m_pFontManager->createFont ( szFontName, szFontFile.c_str(), uSize, uFlags, bAutoScale, 1024, 768 );
        }
...
{
    CEGUI::String glyphSet; // (?) we need temporary (CEGUI::String) string to define our glyphs correctly

...
abiString CGUIElement_Impl::GetText ( void )
{
...
abiString CGUIElement_Impl::GetFont ( void )
{
    try
    {
...
abiString CGUIElement_Impl::GetProperty ( const char *szProperty )
{
    CEGUI::String strValue;
    try
    {
        // Return the string. std::string will copy it
        CEGUI::String propString = m_pWindow->getProperty ( CGUI_Impl::GetUTFString ( szProperty ) );

        strValue = CGUI_Impl::GetUTFString( propString.c_str() );
    }
    catch ( CEGUI::Exception e )
    {}

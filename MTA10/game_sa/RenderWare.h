...
typedef void                (__cdecl*RwSetDeviceStateNative_t) ( eRwDeviceCmd cmd, rwDeviceValue_t value );
typedef void                (__cdecl*RwGetDeviceStateNative_t) ( eRwDeviceCmd cmd, rwDeviceValue_t& value );

typedef int                 (__cdecl*RwRasterCreateNative_t) ( void *userdata, RwRaster *rasterObj, unsigned int combinedFlags );
...
struct RwInterface   // size: 1456
{
    ...
    BYTE                    m_pad[8];                                       // 24
    RwSetDeviceStateNative_t    m_deviceCommand;                            // 32
    RwGetDeviceStateNative_t    m_getDeviceCommand;                         // 36
    BYTE                    m_pad20[48];                                    // 40

    RwRasterCreateNative_t  m_rasterCreateNative;                           // 88
    BYTE                    m_pad23[20];                                    // 92

...
inline size_t GetVehicleSuspensionLineBufferSize( CColDataSA *colData, CVehicleSAInterface *pVehicle )
{
    ...
    if ( vehicleType == VEHICLE_MONSTERTRUCK )
    {
        ...
        // Do some asserts.
        assert( colData->hasSuspensionCones == true );
    }
    else if ( vehicleType == VEHICLE_BIKE )
    {
        numWheels = 4;

        assert( colData->hasSuspensionCones == false );
    }
    ...
    // Get the true allocation size for this collision data.
    allocSize = numWheels * colData->GetSuspensionLineSize();
...
static void InitializeVehiclePublicSuspensionLines( CVehicleSAInterface *pVehicle, CColDataSA *colData )
{
    ...
    if ( bufSize != 0 )
    {
        CColSuspensionDataSA *suspLineBuf = (CColSuspensionDataSA*)RwMalloc( bufSize, 0 );

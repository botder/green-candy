...
// Binary offsets: (1.0 US and 1.0 EU): 0x0040FB60
CColModelSAInterface::CColModelSAInterface( void )
{
    // Initialize common fields.
    this->m_colPoolIndex = 0;   // 'generic' pool index; basically belongs to no sector.
    this->pColData = NULL;

    this->m_isCollidable = false;
    this->m_isColDataSegmented = false;
    this->m_releaseDataOnDestroy = true;
}
...
// Binary offsets: (1.0 US and 1.0 EU): 0x0040F740
void CColModelSAInterface::UnsegmentizeData( void )
{
    // If the col data is segmented, we gotta rellocate it.
    // This is required for certain parts in the engine where the col data must not be immutable.
    if ( this->m_isColDataSegmented )
    {
        // Allocate an own memory block for the colData header.
        CColDataSA *colData = new (_mallocNew( sizeof( CColDataSA ) )) CColDataSA;

        // Clone over the colData from us to the new.
        // The only way to clone is to allocate unsegmentized pointers.
        colData->Clone( this->pColData );

        // Delete the old data.
        _freeMemGame( this->pColData );

        // Store the new pointer.
        this->pColData = colData;

        // We no longer are segmented.
        this->m_isColDataSegmented = false;
    }

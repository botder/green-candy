...
inline void YieldStreamingFiber( NativeExecutive::CFiber *fiber )
{
...
struct corotRwLoadHandler
{
    ...
    inline corotRwLoadHandler( void *buf, size_t bufSize, NativeExecutive::CFiber *fiber )
    {
        // Set up the RenderWare buffer
...
{
    __forceinline corotPulseManager( NativeExecutive::CFiber *fiber ) : m_fiber( fiber )
    {
    }
    ...
    NativeExecutive::CFiber *m_fiber;
};

void __stdcall LoaderFiberRuntime( NativeExecutive::CFiber *fiber, void *ud )
{
...
namespace Streaming
{
    NativeExecutive::CExecutiveGroup *fiberGroup = NULL;
};

void Streaming::LeaveFiberMode( void )
{
    NativeExecutive::CExecutiveManager *fiberMan = pGame->GetExecutiveManager();
    ...
    if ( enableFiberedLoading )
    {
        ...
        for ( unsigned int n = 0; n < MAX_STREAMING_REQUESTERS; n++ )
        {
            ...
            if ( NativeExecutive::CFiber *fiber = requester.loaderFiber )
            {
        ...
        loaderPerfMultiplier = fiberGroup->GetPerfMultiplier();
...
void Streaming::EnterFiberMode( void )
{
    ...
    if ( enableFiberedLoading )
    {
        ...
        for ( unsigned int n = 0; n < MAX_STREAMING_REQUESTERS; n++ )
        {
            ...
            NativeExecutive::CFiber *newFiber = fiberMan->CreateFiber( LoaderFiberRuntime, (void*)n );
...
{
    return ( fiberGroup ) ? fiberGroup->GetPerfMultiplier() : loaderPerfMultiplier;
}
